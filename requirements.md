...
1. **Skill 安装**：手动将 `skills/` 目录复制到目标项目的 `.claude/skills/`
...
5. **日志轮转**：`log.md` 的 entry 标题数量达到 150 时，自动归档到当月的 `log-YYYY-MM.md`（当月内追加合并）；若当月归档文件过大则分卷为 `log-YYYY-MM-NN.md`，并新建空日志
...
├── .claude/
│   ├── skills/
│   │   ├── whylog-record.md   # 记录 skill
│   │   └── whylog-review.md   # 分析 skill
└── docs/
    └── decisions/
